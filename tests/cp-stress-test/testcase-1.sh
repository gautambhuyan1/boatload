...
images=("quay.io/redhat-performance/test-gohttp-probe:v0.0.2" "gcr.io/google_containers/pause-amd64:3.0" "quay.io/akrzos/hello-kubernetes:20210907")
tc_titles=("gohttp" "pause" "hello-kubernetes")
csv_titles=("gohttp" "pause" "hello-kubernetes")
for (( arg_index=0; arg_index<${#images[@]}; arg_index++)); do
  for iteration in `seq 1 ${iterations}` ; do
    test_index=$((${test_index} + 1))
    echo "$(date -u +%Y%m%d-%H%M%S) - node density ${tc_num}.${test_index} - ${iteration}/${iterations} - ${total_pods} namespaces, 1 deploy, 1 pod, 2 container, ${tc_titles[$arg_index]} image, 1 service, 1 route, no probes, no configmaps, no secrets, no resources set"
    logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-nodedensity-${tc_num}.${test_index}.log"
    ../../boatload/boatload.py ${dryrun} ${csvfile} --csv-title "${total_pods}n-1d-1p-2c-${csv_titles[$arg_index]}-${iteration}" -n ${total_pods} -d 1 -p 1 -c 2 -l -r --no-probes -i ${images[$arg_index]} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile}
    echo "$(date -u +%Y%m%d-%H%M%S) - node density ${tc_num}.${test_index} - ${iteration}/${iterations} complete, sleeping ${sleep_period}"
    sleep ${sleep_period}
    echo "****************************************************************************************************************************************"
  done
