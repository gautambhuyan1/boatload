...
#   echo "$(date -u +%Y%m%d-%H%M%S) - node density ${tc_num}.${test_index} - ${iteration}/${iterations} - ${total_pods} namespaces, 1 deploy, 1 pod, 1 container, gohttp image, 1 service, 1 route, http probes, 4 configmaps, 4 secrets, mixedresources "
#   logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-nodedensity-${tc_num}.${test_index}.log"
#   source namespace-create.sh
#   sleep 500
#   ../../boatload/boatload-mixed-pv-test-2.py ${dryrun} ${csvfile} --csv-title "${total_pods}n-1d-1p-1c-gubu-pv-probes-cm-s-${iteration}" -n ${total_pods} -d 1 -p 1 -c 1 -v 1 -l -r ${probes} ${resources} ${configmaps_secrets} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile} --enable-pod-annotations -a ${annotations}
...
for iteration in `seq 1 ${iterations}`; do
  ...
  annotations=" cpu-load-balancing.crio.io=\''true'\' irq-load-balancing.crio.io=\''disable'\' cpu-quota.crio.io=\''disable'\' "
  configmaps_secrets=" -m 4 --secrets 4 "
#  probes=" --startup-probe http,0,10,2,12,1 --liveness-probe http,0,10,2,5,1 --readiness-probe http,0,10,2,5,1 "
...
#  source namespace-create.sh
#  sleep 500
#  ../../boatload/boatload-mixed-pv-test-2.py ${dryrun} ${csvfile} --csv-title "${total_pods}n-1d-1p-1c-gubu-pv-probes-4cm-4s-${iteration}" -n ${total_pods} -d 1 -p 1 -c 1 -v 1 -l -r ${probes} ${resources} ${configmaps_secrets} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile} --enable-pod-annotations -a ${annotations}
#  oc delete $(oc get pv -o name)
