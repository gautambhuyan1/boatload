...
#  test_index=$((${test_index} + 1))
#  probes=" --startup-probe http,0,1,1,12 --liveness-probe http,0,1,1,3 --readiness-probe http,0,1,1,3,1 "
#  annotations=" cpu-load-balancing.crio.io=\''true'\' irq-load-balancing.crio.io=\''disable'\' "
...
#  logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-podchurn-${tc_num}.${test_index}.log"
#  ../../boatload/boatload-churn.py ${dryrun} ${csvfile} --csv-title "${total_160_pods}n-1d-1p-3c-${iteration}" ${metrics} -n ${total_160_pods} -d 1 -p 1 -c 2 -l -r ${probes} ${gohttp_env_vars} ${resources} ${INDEX_ARGS} &> ${logfile} --no-measurement-phase --enable-pod-annotations -a ${annotations}
#  echo "$(date -u +%Y%m%d-%H%M%S) - pod churn ${tc_num}.${test_index} - ${iteration}/${iterations} complete, sleeping ${sleep_period}"
#  sleep ${sleep_period}
#  echo "****************************************************************************************************************************************"
#done
...
#  logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-podchurn-${tc_num}.${test_index}.log"
#  ../../boatload/boatload-churn.py ${dryrun} ${csvfile} --csv-title "${total_225_pods}n-1d-1p-1c-gu-${iteration}" ${metrics} -n ${total_225_pods} -d 1 -p 1 -c 1 -l -r --no-probes ${resources} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile} --no-measurement-phase
#  echo "$(date -u +%Y%m%d-%H%M%S) - pod churn ${tc_num}.${test_index} - ${iteration}/${iterations} complete, sleeping ${sleep_period}"
#  sleep ${sleep_period}
#  echo "****************************************************************************************************************************************"
#done
...
#  logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-podchurn-${tc_num}.${test_index}.log"
#  ../../boatload/boatload-churn.py ${dryrun} ${csvfile} --csv-title "${total_225_pods}n-1d-1p-1c-8cm-8s-${iteration}" ${metrics} -n ${total_225_pods} -d 1 -p 1 -c 1 -l -r -m 8 --secrets 8 --no-probes ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile} --no-measurement-phase
#  echo "$(date -u +%Y%m%d-%H%M%S) - pod churn ${tc_num}.${test_index} - ${iteration}/${iterations} complete, sleeping ${sleep_period}"
#  sleep ${sleep_period}
#  echo "****************************************************************************************************************************************"
#done
...
#  logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-podchurn-${tc_num}.${test_index}.log"
#  ../../boatload/boatload-churn.py ${dryrun} ${csvfile} --csv-title "${total_225_pods}n-1d-1p-1c-tcp-s1-l1-r1-${iteration}" ${metrics} -n ${total_225_pods} -d 1 -p 1 -c 1 -l -r ${probes} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile} --no-measurement-phase
#  echo "$(date -u +%Y%m%d-%H%M%S) - pod churn ${tc_num}.${test_index} - ${iteration}/${iterations} complete, sleeping ${sleep_period}"
#  sleep ${sleep_period}
#  echo "****************************************************************************************************************************************"
#done
...
#  test_index=$((${test_index} + 1))
#  probes=" --startup-probe http,0,1,1,12 --liveness-probe http,0,1,1,3 --readiness-probe http,0,1,1,3,1 "
#  echo "$(date -u +%Y%m%d-%H%M%S) - pod churn ${tc_num}.${test_index} - ${iteration}/${iterations} - ${total_225_pods} namespaces, 1 deploy, 1 pod, 1 container, gohttp image, 1 service, 1 route, ${probes}, 8 configmaps, 8 secrets, no resources set"
#  logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-podchurn-${tc_num}.${test_index}.log"
#  ../../boatload/boatload-churn.py ${dryrun} ${csvfile} --csv-title "${total_225_pods}n-1d-1p-1c-http-s1-l1-r1-${iteration}" ${metrics} -n ${total_225_pods} -d 1 -p 1 -c 1 -l -m 8 --secrets 8 -r ${probes} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile} --no-measurement-phase
#  echo "$(date -u +%Y%m%d-%H%M%S) - pod churn ${tc_num}.${test_index} - ${iteration}/${iterations} complete, sleeping ${sleep_period}"
#  sleep ${sleep_period}
#  echo "****************************************************************************************************************************************"
#done
...
#  echo "$(date -u +%Y%m%d-%H%M%S) - pod churn ${tc_num}.${test_index} - ${iteration}/${iterations} - ${total_225_pods} namespaces, 1 deploy, 1 pod, 1 container, gohttp image, 1 service, 1 route, ${probes}, no configmaps, no secrets, no resources set"
#  logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-podchurn-${tc_num}.${test_index}.log"
#  ../../boatload/boatload-churn.py ${dryrun} ${csvfile} --csv-title "${total_225_pods}n-1d-1p-1c-exec-s10-l10-r10-${iteration}" ${metrics} -n ${total_225_pods} -d 1 -p 1 -c 1 -l -r ${probes} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile} --no-measurement-phase
#  echo "$(date -u +%Y%m%d-%H%M%S) - pod churn ${tc_num}.${test_index} - ${iteration}/${iterations} complete, sleeping ${sleep_period}"
#  sleep ${sleep_period}
#  echo "****************************************************************************************************************************************"
#done
...
#  logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-podchurn-${tc_num}.${test_index}.log"
#  ../../boatload/boatload-churn.py ${dryrun} ${csvfile} --csv-title "${total_225_pods}n-1d-1p-1c-${iteration}" ${metrics} -n ${total_225_pods} -d 1 -p 1 -c 1 -l -r ${configmaps_secrets} ${probes} ${resources} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile} --no-measurement-phase
#  echo "$(date -u +%Y%m%d-%H%M%S) - pod churn ${tc_num}.${test_index} - ${iteration}/${iterations} complete, sleeping ${sleep_period}"
#  sleep ${sleep_period}
#  echo "****************************************************************************************************************************************"
#done
...
#  echo "$(date -u +%Y%m%d-%H%M%S) - pod churn ${tc_num}.${test_index} - ${iteration}/${iterations} - ${total_160_pods} namespaces, 1 deploy, 1 pod, 1 container, gohttp image, 1 service, 1 route, http probes, 8 configmaps, 8 secrets, guaranteed resources"
#  logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-podchurn-${tc_num}.${test_index}.log"
#  ../../boatload/boatload-churn.py ${dryrun} ${csvfile} --csv-title "${total_160_pods}n-1d-1p-1c-${iteration}" ${metrics} -n ${total_160_pods} -d 1 -p 1 -c 1 -l -r ${configmaps_secrets} ${probes} ${resources} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile} --enable-pod-annotations -a ${annotations} --no-measurement-phase
#  echo "$(date -u +%Y%m%d-%H%M%S) - pod churn ${tc_num}.${test_index} - ${iteration}/${iterations} complete, sleeping ${sleep_period}"
#  sleep ${sleep_period}
#  echo "****************************************************************************************************************************************"
#  done
...
#  echo "$(date -u +%Y%m%d-%H%M%S) - pod churn ${tc_num}.${test_index} - ${iteration}/${iterations} - ${total_225_pods} namespaces, 1 deploy, 1 pod, 1 container, gohttp image, 1 service, 1 route, http probes, 8 configmaps, 8 secrets, guaranteed resources"
#  logfile="../logs/$(date -u +%Y%m%d-%H%M%S)-podchurn-${tc_num}.${test_index}.log"
#  ../../boatload/boatload-churn.py ${dryrun} ${csvfile} --csv-title "${total_225_pods}n-1d-1p-1c-${iteration}" ${metrics} -n ${total_225_pods} -d 1 -p 1 -c 1 -l -r ${configmaps_secrets} ${probes} ${resources} ${gohttp_env_vars} ${measurement} ${INDEX_ARGS} &> ${logfile} --enable-pod-annotations -a ${annotations} --no-measurement-phase
#  echo "$(date -u +%Y%m%d-%H%M%S) - pod churn ${tc_num}.${test_index} - ${iteration}/${iterations} complete, sleeping ${sleep_period}"
#  sleep ${sleep_period}
#  echo "****************************************************************************************************************************************"
#  done
